...
export default function NowPlaying() {
  ...
  function get() {
    fetch('/api/now-playing')
      .then((d) => d.json())
      .then((newData) => {
        if (newData.songUrl !== data.songUrl) setData(newData)
      })
  }
  ...
  return (
    <div className="flex flex-row-reverse items-center sm:flex-row mb-8 space-x-0 sm:space-x-2 w-full">
      ...
      <div className="inline-flex flex-col sm:flex-row w-full max-w-full truncate">
        {data?.songUrl ? (
          <a
            className="capsize text-gray-400 dark:text-gray-200 font-medium  max-w-max truncate"
            href={data.songUrl}
            target="_blank"
          ...
          <p className="capsize text-gray-400 font-medium">Not Playing</p>
        )}
        <span className="capsize mx-2 text-gray-500 dark:text-gray-300 hidden sm:block">
          {' – '}
        </span>
        <p className="capsize text-gray-200 max-w-max truncate">
          {data?.artist ?? 'Spotify'}
        </p>
      </div>
    </div>
